...
class PokemonTracker:
    ...
    def find_minimap_in_frame(self, screen):
        """Find minimap region in current frame using white detection"""
        hsv = cv2.cvtColor(screen, cv2.COLOR_BGR2HSV)

        # Detect white pixels (Pokemon markers have white centers)
        ...
        # Get bounding box of all white regions (likely the minimap area)
        all_points = np.vstack(contours)
        x, y, w, h = cv2.boundingRect(all_points)

        # Expand region slightly and ensure it's reasonable size
        padding = 30
        x = max(0, x - padding)
        y = max(0, y - padding)
        ...
        # Minimap should be roughly square and reasonable size
